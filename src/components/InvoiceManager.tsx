...
export default function InvoiceManager() {
  ...
  const [showValidationModal, setShowValidationModal] = useState<number | null>(null);
  ...
  const fetchInvoices = async (projectId: number) => {
    setLoading(true);
    try {
  ...
  const handleCreateInvoice = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (selectedProject) fetchInvoices(selectedProject);
  ...
  const validateEInvoice = async (invoiceId: number) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/invoices/${invoiceId}/validate-einvoice`);
      const data = await res.json();
      setValidationResult(data);
      setShowValidationModal(invoiceId);
    } catch (err) {
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            ...
            <h3 className="text-2xl font-black text-brand-dark tracking-tighter">
              {selectedProject ? 'Projekt-Rechnungen' : 'Alle Rechnungen'}
            </h3>
          </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          {invoices.length === 0 ? (
            ...
            invoices.map(invoice => (
              <div key={invoice.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex justify-between items-center group hover:border-brand-primary/30 transition-all">
                <div className="flex items-center gap-6">
                  <div className="w-14 h-14 bg-slate-100 rounded-2xl flex items-center justify-center text-brand-dark shadow-sm">
                    <Receipt size={28} />
                  </div>
                  <div>
                    <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-4 text-xs font-bold uppercase tracking-widest mt-1">
                      ...
                      <span className="text-slate-400">Fällig: {new Date(invoice.due_date).toLocaleDateString('de-DE')}</span>
                  ...
                  <div className="flex items-center gap-12">
                    ...
                    <div className="flex gap-2">
                      <button 
                        onClick={() => validateEInvoice(invoice.id)}
                      ...
                      <button 
                        onClick={() => window.open(`/api/invoices/${invoice.id}/export-zugferd`, '_blank')}
